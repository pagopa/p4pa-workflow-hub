...
import it.gov.pagopa.pu.workflow.exception.WorkflowExceptionHandler;
import it.gov.pagopa.pu.workflow.exception.transcoder.handler.ConstraintViolationExceptionMessageTranscoderTest;
...
import static it.gov.pagopa.pu.workflow.exception.transcoder.handler.ConstraintViolationExceptionMessageTranscoderTest.EXPECTED_CONSTRAINT_EXCEPTION_MESSAGE_TRANSCODED;
...
public abstract class CommonExceptionHandlerTest {
  ...
  void handleAuthorizationDeniedException() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      .andExpect(MockMvcResultMatchers.status().isForbidden())
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("WORKFLOW_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("WORKFLOW_FORBIDDEN"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.message").value("Error"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.fields").doesNotExist())
      .andExpect(MockMvcResultMatchers.jsonPath("$.traceId").value(traceId));
  }
  ...
  void handleHttpHostConnectionExceptionException() throws Exception {
    ...
    performRequest(DATA, MediaType.APPLICATION_JSON)
      .andExpect(MockMvcResultMatchers.status().isInternalServerError())
      .andExpect(MockMvcResultMatchers.jsonPath("$.category").value("WORKFLOW_GENERIC_ERROR"))
      .andExpect(MockMvcResultMatchers.jsonPath("$.code").value("WORKFLOW_CONNECTION_ERROR"))
...
//region Spring Data Rest
  ...
  void handleCrudResourceBaseBusinessException() throws Exception {
    Long id = -12L;
    doThrow(new InvalidValueException("ERROR", "message", List.of(new PuErrorDTO.ErrorFieldDTO("fieldName", "resolvedError", "resolved message")))).when(testCrudControllerSpy).testCrudEndpoint(id);
