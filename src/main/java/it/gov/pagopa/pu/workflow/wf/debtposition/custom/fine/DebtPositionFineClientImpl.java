...
import it.gov.pagopa.pu.workflow.wf.debtposition.custom.fine.wfreductioneexpiration.FineReductionOptionExpirationWF;
...
public class DebtPositionFineClientImpl implements DebtPositionFineClient {
  ...
  public String handleFineReductionExpiration(Long debtPositionId, PaymentEventRequestDTO paymentEventRequestDTO, FineWfExecutionConfig executionParams) {
    ...
    FineReductionOptionExpirationWF workflow = workflowService.buildWorkflowStub(
      ...
      taskQueue,
      workflowId);
    WorkflowClient.start(workflow::handleFineReductionExpiration, debtPositionId, paymentEventRequestDTO, executionParams);
    return workflowId;
  }
}
